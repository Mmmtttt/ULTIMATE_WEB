...
2. 视频接口：
   ...
   - /api/v1/video/actor/works/<actor_id>
   - /api/v1/video/actor/works-cache/clear
3. 清单接口：
   - /api/v1/list/platform/lists
   - /api/v1/list/platform/list/detail
   - /api/v1/list/import
   - /api/v1/list/sync
   - /api/v1/list/import/favorites
   - /api/v1/list/sync/favorites
4. 作者/演员接口：
   - /api/v1/author/search-works
   - /api/v1/author/check-updates
   - /api/v1/author/new-works
   - /api/v1/author/works/<author_id>
   - /api/v1/actor/search-works
   - /api/v1/actor/check-updates
   - /api/v1/actor/new-works
   - /api/v1/actor/works/<actor_id>
   - /api/v1/actor/videos
5. 推荐与配置接口：
...
## 11) Third-party Extension Prompt (2026-03-23 Latest)
```text
Please extend coverage under tests/features/third_party_integration/ and follow this order:
1. First output a gap list:
   - Already covered endpoints
   - Newly discovered uncovered branches
   - Planned new test cases (with file paths)
2. Every test case MUST include a full "Case Description" before the test body:
   - Purpose
   - Steps
   - Expected Result
   - History Changes (YYYY-MM-DD + note)
3. Prefer backend integration for third-party contracts:
   - Verify adapter/service call style
   - Verify critical input parameters
   - Verify response/data mapping into API output and isolated JSON persistence
4. Required high-priority branches:
   - import fallback branches (`get_video_by_code`, duplicate guards in home/recommendation)
   - search pagination/next-page branches
   - preview refresh/download branches
   - proxy/play-url forwarding branches
   - list import/sync de-dup branches
   - author/actor update-check persistence branches
   - recommendation cache download contract branches
   - config data_dir migration hook branches
5. E2E scope (only for stable pages/selectors):
   - User click flow must trigger backend request with asserted query/body.
   - For playback pages, MUST assert full chain:
     user click `.video-preview` -> `/play-urls` request -> player visible -> `video.src` maps to `/api/v1/video/proxy2?...`.
6. Never use production data/network:
   - Use isolated runtime generated by tests/tools/prepare_test_env.py
   - Use mock/stub for third-party dependencies.
7. Final output must include:
   - changed files
   - coverage points
   - run commands
   - failure localization suggestions
```
