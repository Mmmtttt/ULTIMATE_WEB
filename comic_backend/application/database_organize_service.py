...
class DatabaseOrganizeService:
    ...
    def __init__(self, comic_service: ComicAppService = None):
    ...
    def get_options(self, mode: str) -> ServiceResult:
        ...
        if normalized_mode == "comic":
            ...
            options = [
                {
                    "action": "video_feature_placeholder",
                    "name": "视频数据库整理（建设中）",
                    "description": "视频模式整理能力将在后续版本开放。",
                    "confirm_message": "该功能尚未实现。",
                    "implemented": False,
                }
            ]
    ...
    def run(self, mode: str, action: str) -> ServiceResult:
        ...
        if normalized_mode == "video":
            return ServiceResult.error("视频数据库整理功能尚未实现")
